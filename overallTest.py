...
def split_video_ffmpeg(input_file, output_folder, target_chunk_size_MB_param, segment_duration):
    ...
    if duration % segment_duration > 0:
        num_segments += 1
    print("Number of Segments to be divided", num_segments)

    # Generate and execute ffmpeg commands for each segment
    for i in range(int(num_segments)):
        ...
        # Run FFmpeg command to create the segment
        subprocess.run([
            'ffmpeg',
            '-i', input_file,
            '-c', 'copy',
...
if __name__ == "__main__":
    ...
    os.remove(f'chunks_of_{os.path.basename(video_path)[:-4]}')
